                ...
                <div class="card">
                    <div class="card-body">
                     <form method="POST" accept-charset="UTF-8"
                        class="storeUser" id="submit-form" autocomplete="off" action="{{ route('tasks.update', encrypt($task->id)) }}" enctype="multipart/form-data">
                            {{csrf_field()}}
                            @method('PUT')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        id="priority" name="priority" required>
                                            ...
                                            {{old('priority',$task->priority) == $priority['low'] ?"selected" : ''}}>
                                            Low</option>
                                            <option value="{{ $priority['medium'] }}"
                                            {{old('priority',$task->priority) == $priority['medium'] ?"selected" : ''}}>
                                            Medium</option>
                                            <option value="{{ $priority['high'] }}"
                                            {{old('priority',$task->priority) == $priority['high'] ?"selected" : ''}}>
                                            High</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        id="start_date" name="start_date" value="{{ old('start_date',date('d-m-Y',strtotime($task->start_date))) }}"
                                        required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="file" id="file" data-default-file="{{ !empty($task->file) ? url($task->file) : '' }}"
                                ...
                                <div class="col-md-6">
                                        ...
                                        id="project" name="project" required>
                                            ...
                                            @if(count($project) > 0)
                                                @foreach($project as $row)
                                                    <option value="{{ $row->id }}" {{ old('project_id', $task->project_id) == $row->id ? 'selected' : '' }}>{{ $row->name }}</option>
                                                @endforeach
                                            @else
                                    ...
                                    <div class="form-group">
                                        ...
                                        id="status" name="status" required>
                                            ...
                                            {{ old('status',$task->status) == $status['todo'] ?"selected" : ''}}>
                                            To Do</option>
                                            <option value="{{ $status['inprogress'] }}"
                                            {{ old('status',$task->status) == $status['inprogress'] ?"selected" : ''}}>
                                            In Progress</option>
                                            <option value="{{ $status['done'] }}"
                                            {{ old('status',$task->status) == $status['done'] ?"selected" : ''}}>
                                            Done</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        id="start_date" name="deadline" value="{{ old('deadline',date('d-m-Y',strtotime($task->deadline))) }}" required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        name="description" maxlength="500" rows="4" required>{{ old('description', $task->description) }}</textarea>
